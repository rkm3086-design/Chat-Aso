...
-- حاوية الإشعارات الجانبية (بجانب العلامة الحمراء) - تم توسيعها قليلاً لتناسب الحجم الجديد
...
PopupHolder.Position = UDim2.new(1, -250, 0.5, 40)
PopupHolder.Size = UDim2.new(0, 240, 0, 250)
...
-- إشعار جانبي (بحجم أكبر قليلاً، مع تأثير التلاشي التدريجي Fade Out)
local function showSidePopup(senderId, senderName, text, isSticker, stickerId)
    task.spawn(function()
        ...
        popup.Size = UDim2.new(1, 0, 0, 44) -- تم تكبير الطول قليلاً
        ...
        avatarImg.Size = UDim2.new(0, 32, 0, 32) -- تكبير الأفاتار قليلاً
        ...
        if isSticker then
            ...
            img.Parent = popup
            img.Position = UDim2.new(0, 44, 0, 6)
            ...
            contentLabel.Position = UDim2.new(0, 44, 0, 4)
            contentLabel.Size = UDim2.new(1, -50, 1, -8)
            contentLabel.BackgroundTransparency = 1
            contentLabel.TextXAlignment = Enum.TextXAlignment.Left
            contentLabel.Font = Enum.Font.SourceSansBold
            contentLabel.TextSize = 13 -- تكبير حجم خط النص قليلاً
            contentLabel.TextWrapped = true
            contentLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            contentLabel.Text = "[" .. senderName .. "]: " .. (text or "")
        end
        
        -- الانتظار لمدة 3.5 ثانية ثم بدء التلاشي
        task.wait(3.5)
        
        -- تأثير التلاشي (Fade Out)
        local tweenInfo = TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
        
        local t1 = TweenService:Create(popup, tweenInfo, {BackgroundTransparency = 1})
        local t2 = TweenService:Create(stroke, tweenInfo, {Transparency = 1})
        local t3 = TweenService:Create(avatarImg, tweenInfo, {ImageTransparency = 1})
        
        t1:Play()
        t2:Play()
        t3:Play()
        
        if not isSticker and popup:FindFirstChild("TextLabel") then
            local t4 = TweenService:Create(popup.TextLabel, tweenInfo, {TextTransparency = 1})
            t4:Play()
        elseif isSticker and popup:FindFirstChild("ImageLabel", true) then
            -- إذا كان ملصق
            for _, child in pairs(popup:GetChildren()) do
                if child:IsA("ImageLabel") and child ~= avatarImg then
...
-- Display Functions داخل الشات الرئيسي
local function addMessageToUI(senderId, senderName, text, isSticker, stickerId, isSystem, systemColor)
    ...
    if isSystem then
        ...
        if isSticker then
            ...
            img.Position = UDim2.new(0, 34, 0, 0)
            ...
            textLabel.Text = "[" .. senderName .. "]: " .. (text or "")
